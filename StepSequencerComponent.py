...
class NoteSelectorComponent(ControlSurfaceComponent):
	...
	def _is_mute_shifted(self):
		return self._stepsequencer._is_mute_shifted

	@property
	def _is_velocity_shifted(self):
		return self._stepsequencer._is_velocity_shifted

	@property
	def _drum_group_device(self):
		return None
		#self._stepsequencer._drum_group_device
	...
	def is_chromatic(self):
		return self._stepsequencer._scale_component.is_chromatic

	@property
	def is_diatonic(self):
		return self._stepsequencer._scale_component.is_diatonic
	...
	def note_offset_button_value(self, value, sender):
		if self.is_enabled() and value > 0 and self._enable_buttons:
			if self._is_mute_shifted:
				try:
					index = self._buttons.index(sender)
				except ValueError:
					index = -1
				self._stepsequencer._note_editor.mute_lane(self._root_note + index)
			else:
				try:
	...
	def update(self):
		if self.is_enabled():
			self._stepsequencer._track_controller._implicit_arm = self._is_velocity_shifted and not self._stepsequencer._is_locked
			if self._stepsequencer._track_controller._implicit_arm:
			...
			if self._is_velocity_shifted and not self._stepsequencer._is_locked:
				...
				# set goblaly in my script. #self._control_surface._c_instance.set_feedback_velocity(5)#self._skin.scale.note_recording)
	...
	def _update_buttons(self):
		if self._enable_buttons and self.is_enabled() and self._buttons!=None:
			for i in range(len(self._buttons)):
				if self._clip == None:
					self._buttons[i].set_on_off_values("DefaultButton.Disabled", "DefaultButton.Disabled")
					...
					if self._is_velocity_shifted and not self._stepsequencer._is_locked:
						...
						# self._buttons[i].turn_off()
						self._buttons[i].set_enabled(False)
						self._buttons[i].set_channel(11)
						self._buttons[i].set_identifier(note)
					else:
						self._buttons[i].set_enabled(True)
						self._buttons[i].use_default_message()
...
class LoopSelectorComponent(ControlSurfaceComponent):
	...
	def _number_of_lines_per_note(self):
		if self._mode == STEPSEQ_MODE_MULTINOTE:
			return self._stepsequencer._number_of_lines_per_note
		else:
			return 1
	...
	@property
	def _mode(self):
		return self._stepsequencer._mode
	...
	@property
	def _is_velocity_shifted(self):
		return self._stepsequencer._note_editor._is_velocity_shifted

	@property
	def _quantization(self):
		return self._stepsequencer._quantization
		...
		if self.is_enabled():
			...
			if value > 0:
				...
				if self._loop_point1 != -1 and self._loop_point2 != -1:
					...
					self._stepsequencer.set_page(self._block)
					self._loop_point1 = -1
					self._loop_point2 = -1
					self.update()
				self.last_button_time = time.time()
...
class StepSequencerComponent(CompoundComponent):
	...
	def set_left_button(self, button):
		self._loop_selector.set_left_button(button)
	
	def set_right_button(self, button):
		self._loop_selector.set_right_button(button)
	
	def set_octave_down_button(self, button):
		self._note_selector.set_down_button(button)
	
	def set_octave_up_button(self, button):
		self._note_selector.set_up_button(button)
	
	def set_velocity_button(self, button):
		self._note_editor.set_velocity_button(button)

	def set_prev_scene_button(self, button):
		self._track_controller.set_prev_scene_button(button)
		
	def set_next_scene_button(self, button):
		self._track_controller.set_next_scene_button(button)
		
	def set_prev_track_button(self, button):
		self._track_controller.set_prev_track_button(button)
		
	def set_next_track_button(self, button):
		self._track_controller.set_next_track_button(button)
...
# SET FUNCTIONS
	...
	def _set_scale_component(self):
		...
		#self._scale_component.set_enabled(False)
		#self._scale_component.set_matrix(self._matrix)
		#self._scale_component._mode = "chromatic"
		#self._scale_component_button = None
		#self.set_scale_button(self._side_buttons[0])
...
# enabled
	def set_enabled(self, enabled):
		...
		if enabled:
			#restore mode backup when coming back
			if self._mode == STEPSEQ_MODE_SCALE_EDIT:
	...
	def set_mode(self, mode, number_of_lines_per_note= -1):
		if number_of_lines_per_note == -1:
			number_of_lines_per_note = self._number_of_lines_per_note
...
# UPDATE
	...
	def _update_scale_component(self):
		self._scale_component.set_enabled(self._mode == STEPSEQ_MODE_SCALE_EDIT)
	...
	def _update_note_editor(self):
		...
		if self._matrix != None:
			if self._mode == STEPSEQ_MODE_NORMAL:
				self._note_editor.set_height(4)#self._matrix._height - 4)
			else:
				self._note_editor.set_height(8)#self._matrix._height)
		...
		if self._clip_slot != None and self._clip_slot.has_clip and self._clip_slot.clip != None and self._clip_slot.clip.is_midi_clip:
			if self._clip == None or self._clip != self._clip_slot.clip:
				...
				if Settings.STEPSEQ__SAVE_SCALE != None and Settings.STEPSEQ__SAVE_SCALE == "clip":  
					...
					self._note_selector.set_scale(self._scale_component.notes, self._scale_component._selected_key)
					#self._control_surface.schedule_message(1, self._note_selector.set_selected_note,(self._scale_component,self._scale_component._octave_index * 12 + self._scale_component._selected_key))
					self._note_selector.set_selected_note(self._scale_component._octave_index * 12 + self._scale_component._selected_key)
...
# DRUM_GROUP_DEVICE
	...
	#	if not self._is_locked:
	#		self._update_drum_group_device()
	#		self._scale_component.set_drumrack(self._drum_group_device != None)
...
# SCALE Selector Button
	def _update_scale_button(self):
		if self.is_enabled():
			if (self._scale_button != None):
				if self._clip != None:
					self._scale_button.set_on_off_values("StepSequencer.Scale.On", "StepSequencer.Scale.Off")
				else:
					self._scale_button.set_on_off_values("DefaultButton.Disabled", "DefaultButton.Disabled")
				if self._mode == STEPSEQ_MODE_SCALE_EDIT:
	...
	def set_scale_button(self, button):
		...
		if (self._scale_button != button):
			if (self._scale_button != None):
				self._scale_button.remove_value_listener(self._scale_button_value)
			self._scale_button = button
			if (self._scale_button != None):
				assert isinstance(button, ButtonElement)
				self._scale_button.add_value_listener(self._scale_button_value)
...
# MUTE SHIFT Button
	...
	def _update_mute_shift_button(self):
		if self.is_enabled() and self._mute_shift_button != None:
			if self._clip != None and self._clip.is_midi_clip:
				self._mute_shift_button.set_on_off_values("StepSequencer.Mute.On", "StepSequencer.Mute.Off")
				if self._is_mute_shifted:
					self._mute_shift_button.turn_on()
				else:
					self._mute_shift_button.turn_off()
			else:
				self._mute_shift_button.set_on_off_values("DefaultButton.Disabled", "DefaultButton.Disabled")
				self._mute_shift_button.turn_off()
	...
	def _update_quantization_button(self):
		if self.is_enabled():
			if (self._quantization_button != None):
				self._quantization_button.set_on_off_values(self.QUANTIZATION_COLOR_MAP[self._quantization_index], "DefaultButton.Disabled")
				if self._clip != None:
...
# LOCK Button
	...
	def set_lock_button(self, button):
		assert (isinstance(button, (ButtonElement, type(None))))
		if (self._lock_button != button):
			if (self._lock_button != None):
				self._lock_button.remove_value_listener(self._lock_button_value)
			self._lock_button = button
			if (self._lock_button != None):
				assert isinstance(button, ButtonElement)
				self._last_lock_button_press = time.time()
				self._lock_button.add_value_listener(self._lock_button_value, identify_sender=True)
			...
			if self._lock_button != None:
				if self._clip != None:
					if self._lock_to_track:
						self._lock_button.set_on_off_values("StepSequencer.Lock.ToTrack.On", "StepSequencer.Lock.ToTrack.Off")
					else:
	...
	def _lock_button_value(self, value, sender):
	...
	def _update_right_button(self):
		if self.is_enabled():
			if self._right_button != None:
				if self._clip != None:
					self._right_button.set_on_off_values("StepSequencer.Nav.On", "StepSequencer.Nav.Off")
					if self._loop_selector.can_scroll(1):
						self._right_button.turn_on()
					else:
						self._right_button.turn_off()
				else:
					self._right_button.set_on_off_values("DefaultButton.Disabled", "DefaultButton.Disabled")
					self._right_button.turn_off()

	def _arrow_right_pressed(self, value, sender):
		if self.is_enabled() and self._clip != None:
	...
	def _update_left_button(self):
		if self.is_enabled():
			if self._left_button != None:
				if self._clip != None:
					self._left_button.set_on_off_values("StepSequencer.Nav.On", "StepSequencer.Nav.Off")
					if self._loop_selector.can_scroll(-1):
						self._left_button.turn_on()
					else:
						self._left_button.turn_off()
				else:
					self._left_button.set_on_off_values("DefaultButton.Disabled", "DefaultButton.Disabled")
					self._left_button.turn_off()

	
	#@arrow_left_button.pressed
	def _arrow_left_pressed(self, button):
		if self._clip != None:
			self._loop_selector.scroll(-1)
			self._note_editor.request_display_page()
			self.update()
